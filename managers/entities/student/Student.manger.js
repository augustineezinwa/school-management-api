module.exports = class Student { 
    ...
    constructor({utils, cache, config, cortex, managers, validators, mongomodels, serializers }={}){
        ...
        this.studentExposed         = [];
        this.httpExposed          = ['post=enrollStudent', 'get=getStudents', 'get=getStudentById', 'patch=updateStudentProfileById', 'delete=deleteStudentById'];
    ...
    async updateStudentProfileById({ __params, admissionNumber, firstName, lastName, dateOfBirth, gender, status }){
        const id = __params.id;
        if(!id) return { errors: 'Student ID is required' };
        ...
        // Data validation
        let result = await this.validators.student.updateStudentProfileById(student);
        if(result) return { errors: result };
